...
void timer_periodic_refresh(void)
{
	...
	for(t = soft_timer_list; t != TIMER_NULL; t = t->next) //每个软件定时器都轮询一遍
	{
		if(t->flag==TIMER_FLAG_RUN) //软件定时器在运行
		{
			if(t->tick_count > 0)
				-- t->tick_count;
			else                     //计时时间到了
			{
				stop_timer(t);      //停止定时器
				t->time_over_proc();//执行对应函数(类似于中断服务函数)
			}
		}
	}
}
